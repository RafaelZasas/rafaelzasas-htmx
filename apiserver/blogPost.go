...
func (api *api) handleBlogPostPage(w http.ResponseWriter, r *http.Request, data *pageData) {
	tmpl := api.newTemplate("views/blog/blogPage.html")
	slug := r.PathValue("slug")
	post, err := api.db.GetPostBySlug(slug)
	...
	if err := api.db.UpdatePostViewCount(post.ID); err != nil {
		log.Println(fmt.Errorf("api.handleBlogPostPage: %w", err))
	}
...
func (api *api) handleBlogEditPage(w http.ResponseWriter, r *http.Request, data *pageData) {
...
func (api *api) handleEditPost(w http.ResponseWriter, r *http.Request, data *pageData) {
	slug := r.PathValue("slug")
	post, err := api.db.GetPostBySlug(slug)
	...
	fmt.Println(post)
	if err := api.db.UpdatePost(post); err != nil {
		log.Println(fmt.Errorf("api.handleEditPost: %w", err))
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		return
	}
